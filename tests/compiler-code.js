...
function createConfiguration() {
    return {
        ...
        sources: {
            'PacktToken.sol': {
                content: fs.readFileSync(path.resolve(__dirname, 'contracts', 'PacktToken.sol'), 'utf8')
            },/*
...
function compileSources(config) {
    try {
			var a = solc.compile(JSON.stringify(config, 1), getImports);
			console.log(a);
      return JSON.parse(a);
    } catch (e) {
        console.log(e);
...
function getImports(dependency) {
    ...
    switch (dependency) {
        case 'PacktToken.sol':
            return {contents: fs.readFileSync(path.resolve(__dirname, 'contracts', 'PacktToken.sol'), 'utf8')};
